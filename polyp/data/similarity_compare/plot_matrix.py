...
try:
    from polyp.version import __version__
except:
    sys.path.append(str(here.parent.parent.parent))
    from polyp.version import __version__
print(f"__version__: {__version__}")

from polyp.data.similarity_compare.dataset_vector import ImageSimilarity
...
def run_overall(imgs_path, dataset):
    ...
    """
    from polyp.data.similarity_compare.dataset_vector import run as run_images_to_vectors
    img_paths = run_images_to_vectors(imgs_path, dataset)

    from polyp.data.similarity_compare.similar_matrix import run as run_vector_to_similarity_matrix
...
if __name__ == "__main__":
    ...
    dataset = "total_polyp_v5_format"
    imgs_path = f"/data/tml/{dataset}/images"
    ...
    target_path = f"/data/tml/mixed_polyp_filtered/{dataset}/images"
    image_similarity = ImageSimilarity()
    delete_list = image_similarity.delete_similar_images(sm, img_paths, target_path, threshold=0.9)
